...
def runCypressTests(allureStashName) {
    script {
        ...
        bat 'npm update'
        try {
            def exitCode = bat(script: 'npx cypress run --headed --record --key 53c9cb4d-fb97-4a4a-9dc6-9f74ea47dd16 --browser firefox --parallel --env allure=true', returnStatus: true)
            if (exitCode != 0) {
