...
def runCypressTests(allureStashName) {
    script {
        ...
        bat 'npm update'
        try {
           def exitCode = bat(script: 'npx cypress run --record false --key 53c9cb4d-fb97-4a4a-9dc6-9f74ea47dd16 --browser chrome --parallel --env allure=true --config-file cypress.config.js --headless', returnStatus: true)
            if (exitCode != 0) {
